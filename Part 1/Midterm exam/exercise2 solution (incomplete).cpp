...
private:
    ...
    void AuxInOrder(CBinTreeNode<T>* n, std::list<T>* L, int &cont);
};
...
{
    int n= 0;
    CBinTree<T>::AuxInOrder(CBinTree<T>::Root(), L, n);
}

template <class T>
void CBinTree<T>::AuxInOrder(CBinTreeNode<T>* n, std::list<T>* L, int &cont)
{
    if ( !n ) {
        cont++;
        return;
    ...
    int i= 0;
    AuxInOrder(n->nodes[0], L, i);
    if(i >= 1) {
        L->push_back(n->value);
    }
    AuxInOrder(n->nodes[1], L, i);
