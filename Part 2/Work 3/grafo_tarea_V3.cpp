...
template <class T, int rows, int columns, int null_char >
class Csparsa {
public:

    typedef Csparsa<T, rows, columns, null_char> self;
    typedef T  Tt;

    T var_nul = null_char;

    nodo_smx<T>* x_[rows];
    nodo_smx<T>* y_[columns];
    ...
    bool ins(int i, int j, T dato);
    bool rem(int i, int j);
    void debug_row(nodo_smx<T>* cent);
...
template <class T, int rows, int columns, int null_char>
inline bool Csparsa<T, rows, columns, null_char>::ins(int i, int j, T dato)
{
    nodo_smx<T>** ptr;
...
template <class T, int rows, int columns, int null_char>
inline bool Csparsa<T, rows, columns, null_char>::rem(int i, int j)
{
    nodo_smx<T>** x;
    nodo_smx<T>** y;
    if (find_y(i, j, y))
...
template <class T, int rows, int columns, int null_char>
inline void Csparsa<T, rows, columns, null_char > ::debug_row(nodo_smx<T>* cent) {
    while (cent) {
        cout << cent->m_data;
        cout << "-> ";
        cent = cent->next;
...
template <class T, int rows, int columns, int null_char >
inline bool Csparsa<T, rows, columns, null_char> ::find_x(int i, int j, nodo_smx<T>**& p) {
    p = &(x_[i]);
    while (*p && (*p)->y_data < j)
        p = &((*p)->next);
    return *p && (*p)->y_data == j;
...
template <class T, int rows, int columns, int null_char >
inline bool Csparsa<T, rows, columns, null_char> ::find_y(int i, int j, nodo_smx<T>**& p) {
    p = &(y_[j]);
    while (*p && (*p)->x_data < i)
        p = &((*p)->down);
    return *p && (*p)->x_data == i;
...
template <class T, int rows, int columns, int null_char>
inline void Csparsa<T, rows, columns, null_char > ::print_matrix_x() {
    cout << "\n";
    for (int i = 0; i < rows; i++) {
        cout << "\t";
        int cont = 0;
...
template <class T, int rows, int columns, int null_char>
nodo_temp<Csparsa<T, rows, columns, null_char >> Csparsa<T, rows, columns, null_char > :: operator()(int i, int j) {
    nodo_smx<T>** p;
    T f;
    if (find_x(i, j, p)) {
        f = (*p)->m_data;
...
void cal(ListGraph<int,double> &g, T num_v){
    ...
    for(int i=1; i<num_v; ++i){ //(int i=1; i<cantidad de vertices; ++i
        for(int j=1; j<num_v; ++j){ //(int j=1; j<cantidad de vertices; ++i
            auto path = sp.path(i, j);
    ...
    for (int i = 1; i < num_v; i++) {
        for (int j = 1; j < num_v; j++) {
            auto path = sp.path(i, j);
            Matrix.ins(i, j, sp.path_size(i, j));
            thread t(Matrix, i, j);
            if(i == j) {
                cout << "\t" << i << "\t-\t" << j << "\t"<< Matrix(i, j) <<  "\t -1" << endl;
            }else {
                cout << "\t" << i << "\t-\t" << j << "\t"<< Matrix(i, j)  << "\t";
                copy(path.begin(), path.end(), ostream_iterator<int>(cout, "->")); cout << endl;
            }
    ...
    cout << "\nla distancia de " << o << " a " << d << " es de: "; cout << Matrix(o, d) << endl;
